...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    ...
    Node<T> sibling = parent;
    Node<T> parent2 = child;

    if (child.isRightChild()) {
      // Left rotation
      if (grandparent != null) {
        grandparent.down[grandparent.down[1] == parent ? 1 : 0] = child;
      } else {
        root = child;
      }
      parent2.up = grandparent;
      sibling.down[1] = child.down[0];
      if (sibling.down[1] != null) {
        sibling.down[1].up = sibling;
      }
      child.down[0] = sibling;
    } else if (child.up != null && child.up.down[0] == child) {
      // Right rotation
      if (grandparent != null) {
        grandparent.down[grandparent.down[0] == parent ? 0 : 1] = child;
      } else {
        root = child;
      }
      parent2.up = grandparent;
      sibling.down[0] = child.down[1];
      if (sibling.down[0] != null) {
        sibling.down[0].up = sibling;
      }
      child.down[1] = sibling;
    } else {
      throw new IllegalArgumentException(
          "provided child and parent node references are not initially (pre-rotation) related that way");
    }
  }
  ...
  public static boolean test1() {
    ...
    bst.insert(60);
    bst.insert(80);
    bst.rotate(bst.findNode(20), bst.findNode(30));
    return bst.toLevelOrderString().equals("[ 50, 20, 70, 30, 60, 80, 40 ]");
  }
  ...
  public static boolean test2() {
    ...
    bst.insert(60);
    bst.insert(80);
    bst.rotate(bst.findNode(60), bst.findNode(70));
    return bst.toLevelOrderString().equals("[ 50, 30, 60, 20, 40, 70, 80 ]");
  }
  ...
  public static boolean test3() {
    ...
    bst.insert(2);
    bst.insert(3);
    bst.rotate(bst.findNode(2), bst.findNode(1));;
    return bst.toLevelOrderString().equals("[ 2, 1, 3 ]");
  }
